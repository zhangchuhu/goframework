...
func InitTurnOverService(conf *config.AppConfig) error {
	if conf == nil || conf.TurnOverThriftAddr == "" || conf.ContractThriftAddr == "" {
		return fmt.Errorf("config not init")
	}
	turnOverThrift = appthrift.NewConnectionPool(20, time.Minute*1, time.Second*3, 1000, conf.TurnOverThriftAddr)
